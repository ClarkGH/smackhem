...
todos:
  ...
  - id: create_position_function
    content: |-
      Create computeCelestialPosition function using out-parameter patt that:

      Rotates light direction, changes color, and adjusts ambient intensity based on simulation time
      Renders visual representations: triangle for sun, square for moon
      Positions sun/moon objects in the sky along the light direction vector
      Shows sun during day, moon during night with smooth transitions
      Orients objects to face toward the scene (perpendicular to light direction)
      Scales objects by distance for consistent visual size
      ern: position = playerPosition + lightDirection * distance
    status: pending
...
- Reaches zenith (overhead) at noon (timeO    // Create collision 
- fDay = 0.5)
